...
Version:      $HopeName: D-lib-dood!walk.dylan(D-kan.19) $
...
define inline function dood-indirect-disk-pointer+object 
 ...
 => (pointer :: <pointer>, disk-object)
  ...
  let found-address
    = ~walk-info-batch?(info) & dood-address(dood, disk-object);
  let address = found-address | attach-object(dood, object, disk-object);
  values(tag-as-address(disk-object, address), disk-object)
end function;
