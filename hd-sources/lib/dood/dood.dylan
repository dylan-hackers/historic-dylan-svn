...
Version:      $HopeName: D-lib-dood!dood.dylan(D-kan.30) $
...
define constant $dood-program-binding-proxy-class-id = 15;
...
define open primary class <dood> (<object>)
  ...
  constant slot dood-world = dood-world-default(), 
    init-keyword: world:;
...
define method dood-new-locator (dood :: <dood>) => (locator)
  override-locator(dood-locator(dood), extension: "new");
end method;
...
define method boot-predefines (dood :: <dood>) => ()
  ...
  register-predefine
    (<dood-program-binding-proxy>, $dood-program-binding-proxy-class-id);
  register-predefine
...
define method initialize
  ...
  when (locator)
    dood-locator(dood) := as(<locator>, locator);
    unless (name)
