...
Version:      $HopeName: D-lib-dood!class.dylan(D-kan.12) $
...
define open generic dood-lazy-getters (class :: <class>);
define open generic dood-weak-getters (class :: <class>);

define method dood-lazy-getters (class :: <class>)
  #()
end method;

define method dood-weak-getters (class :: <class>)
  #()
end method;
...
define method dood-compute-lazy-slot-descriptors 
  ...
  map-as(<dood-slot-sequence-type>, curry(as-dood-slot-descriptor, class),
	 choose(method (sd) member?(sd.slot-getter, lazy-getters) end, 
		dood-instance-slot-descriptors(class)))
end method;
...
define method dood-compute-deep-slot-descriptors 
  ...
  map-as(<dood-slot-sequence-type>, curry(as-dood-slot-descriptor, class),
	 choose(method (sd) 
		  ~member?(sd.slot-getter, weak-getters)
		    & ~member?(sd.slot-getter, lazy-getters) 
		end method, 
		dood-base-deep-slot-descriptors(class)))
end method;
...
define open generic dood-repeated-size 
