...
define sealed method do-get-buffer (the-stream :: <multi-buffered-stream>,
 ...
 => (the-buffer :: <power-of-two-buffer>, eof? :: <boolean>)
  ...
  let new-buffer-next :: <integer> = 
    logand(the-buffer.on-page-bits, the-position);
...
define function preempt-buffer
 ...
 => (buffer-index :: <integer>);
  ...
  if (the-buffer.use-count < 3) 
    the-buffer.use-count := the-buffer.use-count + 1;
  end if;
  ...
  if (the-buffer == $empty-buffer)
    ...
    new-buffer-position := logand(the-buffer.off-page-bits, the-position); 
    ...
    // check first.
    if (the-buffer.owning-stream)
      let the-owning-stream =
	$stream-id-to-stream-map[the-buffer.owning-stream];
      let map-index
    ...
    the-buffer.owning-stream := the-stream.stream-id;
    new-buffer-position := logand(the-buffer.off-page-bits, the-position); 
    let start = 0; let the-size = the-buffer.buffer-size;
...
define sealed method force-output-buffers
  ...
  for (buffer in stream.buffer-vector.buffers)
    if ((buffer.owning-stream == stream.stream-id) & buffer.buffer-dirty?)
...
define method close
  ...
  if (stream-open?(stream))
    if (abort?)
      for (buffer in stream.buffer-vector.buffers)
	if (buffer.owning-stream == stream.stream-id)
...
define function multi-buffered-stream-position-setter
 ...
 => (the-position :: <integer>)
  if (stream-shared-buffer(stream))
    ...
    elseif (logand(the-buffer.off-page-bits, the-position)
