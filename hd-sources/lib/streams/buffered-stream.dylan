...
define generic stream-input-buffer (stream :: <buffered-stream>)
 => (input-buffer :: false-or(<buffer>));

define generic stream-input-buffer-setter 
    (value :: false-or(<buffer>), stream :: <buffered-stream>)
 => (input-buffer :: false-or(<buffer>));


define generic stream-output-buffer (stream :: <buffered-stream>)
 => (output-buffer :: false-or(<buffer>));

define generic stream-output-buffer-setter 
    (value :: false-or(<buffer>), stream :: <buffered-stream>)
 => ( output-buffer :: false-or(<buffer>));
...
define sealed inline method stream-input-buffer (stream :: <buffered-stream>)
 => (input-buffer :: false-or(<buffer>))
  stream.actual-stream-input-buffer
end method;

define sealed inline method stream-input-buffer-setter
    (value ::  false-or(<buffer>), stream :: <buffered-stream>)
 => (input-buffer :: false-or(<buffer>))
  stream.actual-stream-input-buffer := value
end method;

define sealed inline method stream-output-buffer (stream :: <buffered-stream>)
 => (input-buffer :: false-or(<buffer>))
  stream.actual-stream-output-buffer
end method;

define sealed inline method stream-output-buffer-setter
    (value :: false-or(<buffer>), stream :: <buffered-stream>)
 => (input-buffer :: false-or(<buffer>))
   ...
   with-input-buffer (sb = stream)
	  ...
	  if (bi >= ei)
		...
		if (elt == rt)
		  ...
		  let peek-elt = sb & buffer-ref(sb, ni);
		  if (peek-elt == nl)
		    sb.buffer-next := ni + 1
		  end if
		end if
