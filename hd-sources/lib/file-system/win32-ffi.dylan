  ...
  => { begin
	 ...
	 block ()
	   ...
	   if (primitive-machine-word-equal?(primitive-unwrap-machine-word(?wfd),
					     integer-as-raw(0)))
	     win32-file-error("get space for WIN32_FIND_DATA for", "%s", ?directory)
	   end;
	   ?body
	 cleanup
...
define function do-with-file-attributes (file :: <pathname>, f :: <function>)
  let file = expand-pathname(file);
  if (file[size(file) - 1] = $pathname-separator)
    // FindFirstFile requires that there be a filename ...
    file := unsplit-pathname(file, ".")
  end;
...
define function win32-last-error () => (status :: <integer>)
  raw-as-integer
