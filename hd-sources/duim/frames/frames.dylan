...
define method frame-input-focus-setter
 ...
 => (new :: false-or(<sheet>))
  ...
  when (new ~== old)
    note-input-focus-changed(port(frame), old, new);
...
define method note-frame-mapped
  ...
  // Set the initial focus for the frame
  let _port = port(frame);
  let sheet = frame-input-focus(frame);
  note-input-focus-changed(_port, #f, sheet);
  distribute-event(_port, make(<frame-mapped-event>,
			       frame: frame))
...
// Users can use this to apply the function to the given arguments
// in the frame's main thread
// It works by queueing a <function-event> onto the event queue
