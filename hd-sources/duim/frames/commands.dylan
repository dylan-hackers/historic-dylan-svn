...
define method handle-event (client, event :: <command-event>) => ()
  ignore(client);
  let server  = event-frame(event);		//---*** should the server be 'client'?
  let command = event-command(event);
  execute-command-type(command, server: server)
...
define sideways method execute-callback
  ...
  ignore(args);
  let server = sheet-frame(gadget);		//---*** should the server be 'gadget'?
  execute-command-type(command-type, server: server)
end method execute-callback;
...
define sideways method execute-callback
  ...
  ignore(args);
  execute-command-type(command-type, server: frame)
end method execute-callback;
