...
define method do-command-menu-gadgets
    (function :: <function>, frame :: <frame>,
     ...
     #key menu-bar? = #t, tool-bar? = #t) => ()
  local method frob-gadget (sheet) => ()
	  when (gadget?(sheet))
	    let gcommand = gadget-command(sheet);
	    when (gcommand
		  & (gcommand = command
  ...
  when (menu-bar?)
    ...
    when (menu-bar)
      do-sheet-tree(frob-gadget, menu-bar)
    end
  end;
  when (tool-bar?)
    let tool-bar = frame-tool-bar(frame);
    when (tool-bar)
      do-sheet-tree(frob-gadget, tool-bar)
    end
  end;
  for (menu in frame-owned-menus(frame))
    do-sheet-tree(frob-gadget, menu)
  end
