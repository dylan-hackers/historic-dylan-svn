...
define sealed class <standard-pointer> (<pointer>)
  ...
  sealed slot pointer-cursor-override = #f;
...
define thread-slot pointer-cursor-override of <pointer>;
...
define method do-with-busy-cursor
  ...
  if (_port)
    ...
    let old-cursor = pointer-cursor(pointer);
    block ()
      dynamic-bind (pointer-cursor-override(pointer) = cursor)
    ...
    continuation()		// frame is not mapped
  end
end method do-with-busy-cursor;
