...
define method invalidate-cached-regions (sheet :: <sheet>) => ()
  ...
  // Once we hit a mirrored sheet, we know the regions below that sheet
  // are "normalized", so we can stop smashing the cached regions
...
define method invalidate-cached-transforms (sheet :: <sheet>) => ()
  ...
  // Once we hit a mirrored sheet, we know the regions below that sheet
  // are "normalized", so we can stop smashing the cached transforms
  unless (sheet-direct-mirror(sheet))
