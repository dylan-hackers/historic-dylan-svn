...
define method do-distribute-event
  ...
  when (sheet & sheet-mapped?(sheet))
	...
	port-input-focus(_port) := new-focus;
        when (event-class == <button-press-event>
	      & port-focus-policy(_port) == #"click-to-select"
	      & sheet-handles-keyboard?(new-focus))
	  // Notify the back-end even if the focus has not apparently
	  // changed because there might be some mirrored sheets that
	  // grabbed the focus without telling us
	  note-input-focus-changed(_port, old-focus, new-focus)
	end;
...
define method update-focus-for-enter-event
  ...
  let old-focus = port-input-focus(_port);
  port-input-focus(_port) := new-focus;
  when (old-focus ~== new-focus		// don't work so hard...
  ...
  unless (sheet == pointer-sheet(pointer))
    ...
    pointer-cursor(pointer)
      := pointer-cursor-override(pointer) | sheet-cursor(sheet) | #"default"
  end
...
define method update-focus-for-exit-event
  ...
  when (old-focus			// don't work so hard...
	& port-focus-policy(_port) == #"sheet-under-pointer")
    note-input-focus-changed(_port, old-focus, #f)
  end;
