...
define protocol <<port-protocol>> ()
 ...
 => (focus :: false-or(<abstract-sheet>));
  ...
  function note-input-focus-changed
    (port :: <abstract-port>,
     old :: false-or(<abstract-sheet>), new :: false-or(<abstract-sheet>)) => ();
...
define method note-input-focus-changed
    (_port :: <basic-port>,
     old :: false-or(<sheet>), new :: false-or(<sheet>)) => ()
  when (old)
    note-focus-out(_port, old)
  end;
  port-input-focus(_port) := new;	// just in case...
  when (new)
    note-focus-in(_port, new)
  end;
  #f
end method note-input-focus-changed;

define method note-focus-in
    (_port :: <port>, sheet :: <sheet>) => ()
  #f
