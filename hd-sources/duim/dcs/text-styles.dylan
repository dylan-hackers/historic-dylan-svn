...
define protocol <<text-style-protocol>> ()
  ...
  function text-style-components
    (style :: <text-style>)
 => (family, weight :: <text-style-weight>, slant :: <text-style-slant>, size,
...
define sealed inline method make
    (class == <text-style>,
     #key family, weight, slant, size, underline? = #f, strikeout? = #f)
...
define sealed method make-text-style
 ...
 => (text-style :: <standard-text-style>)
  ...
  let face-table   = gethash(family-table, family);
  let face = compute-face-code(weight, slant, underline?, strikeout?);
  unless (face-table)
    face-table := make(<table>);
    gethash(family-table, family) := face-table
  end;
...
define sealed method merge-text-styles
 ...
 => (text-style :: <standard-text-style>)
  let (family1, weight1, slant1, size1, underline?, strikeout?)
    = text-style-components(style);
  if (family1 & weight1 & slant1
      & (size1 & size1 ~== #"larger" & size1 ~== #"smaller"))
    style
  else
    let (family2, weight2, slant2, size2)
