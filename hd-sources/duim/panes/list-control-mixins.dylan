...
define method handle-event
    (pane :: <homegrown-control-mixin>, event :: <button-release-event>) => ()
  when (gadget-enabled?(pane))
    select (event.event-button)
      $right-button =>
	let x = event-x(event);
	let y = event-y(event);
	execute-popup-menu-callback
	  (pane, gadget-client(pane), gadget-id(pane), #f, x: x, y: y);
      $left-button =>
        gadget-selection(pane, do-callback?: #t) := #[];
      otherwise =>
	#f;
    end
  end
...
define method handle-event
    (pane :: <homegrown-control-mixin>, event :: <key-press-event>) => ()
  when (gadget-enabled?(pane))
    execute-key-press-callback
      (pane, gadget-client(pane), gadget-id(pane), event-key-name(event))
  end
end method handle-event;
