...
define method initialize
    (graph :: <graph-control-pane>, #rest initargs,
...
define sealed class <arrow-graph-edge> (<graph-edge-pane>)
  sealed slot %from-head? :: <boolean> = #f,
    init-keyword: from-head?:;
  sealed slot %to-head?   :: <boolean> = #t,
    init-keyword: to-head?:;
...
define sealed method draw-edge
  ...
  when (region-contains-position?(region, x1, y1)
    ...
    draw-arrow(medium, x1, y1, x2, y2,
	       from-head?: edge.%from-head?, to-head?: edge.%to-head?)
  end;
end method draw-edge;
...
define abstract class <graph-control-layout>
    (<standard-repainting-mixin>,
     ...
     <layout-pane>)
  sealed constant slot %graph-control :: <graph-control>,
    required-init-keyword: graph-control:;
...
define method handle-event
    (layout :: <graph-control-layout>, event :: <key-press-event>) => ()
  let control = layout.%graph-control;
...
define method handle-repaint
  ...
  next-method();
  let graph = layout.%graph-control;
