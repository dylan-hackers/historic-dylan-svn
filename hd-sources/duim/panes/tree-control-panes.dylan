...
define sealed method initialize
    (tree :: <tree-control-pane>, #rest initargs,
     #key frame-manager: framem)
  dynamic-extent(initargs);
  next-method();
  with-frame-manager (framem)
    let layout = make(<tree-control-layout>, tree-control: tree);
...
define sealed class <tree-control-layout>
  ...
  sealed constant slot %tree-control :: <tree-control>,
    required-init-keyword: tree-control:;
...
define method handle-event
    (pane :: <tree-control-layout>, event :: <key-press-event>) => ()
  let control = pane.%tree-control;
