...
// This supa dupa version saves up all the fills until the end, and then
// then divides up any leftover space equally amongst them.
...
//--- This code needs to be more careful that each child is within its
//--- constraints when allocating space using ratios.
define function compose-space-for-items
 ...
 => (sizes :: <simple-object-vector>)
  ...
  let stretch? = (desired-size >= size-function(sheet, space-req));
  let give  :: <integer> = 0;
  let extra :: <integer> = 0;
  if (stretch?)
    give  := max-function(sheet, space-req) - size-function(sheet, space-req);
    extra := min(desired-size - size-function(sheet, space-req), give)
  else
    give  := size-function(sheet, space-req) - min-function(sheet, space-req);
    extra := min(size-function(sheet, space-req) - desired-size, give)
  end;
  for (item in items, index :: <integer> from 0)
    let child = sheet?(item) & item;
    if (child & sheet-withdrawn?(child))
      sizes[index] := 0
    else
      let item-sr = item-composer(item);
      if (item-sr == #"fill" | max-function(child | sheet, item-sr) >= $fill)
	let ratio = (ratios & index < size(ratios) & ratios[index]) | 1;
	inc!(ratio-denominator, ratio)
      else
	let preferred-size :: <integer> = size-function(child | sheet, item-sr);
	when (give > 0)
	  let used :: <integer> = 0;
	  if (stretch?)
	    let max-size :: <integer> = max-function(child | sheet, item-sr);
	    used := truncate/((max-size - preferred-size) * extra, give);
	    inc!(preferred-size, used);
	    dec!(give, max-size - preferred-size)
	  else
	    let min-size :: <integer> = min-function(child | sheet, item-sr);
	    used := truncate/((preferred-size - min-size) * extra, give);
	    dec!(preferred-size, used);
	    dec!(give, preferred-size - min-size)
	  end;
	  dec!(extra, used)
	end;
	inc!(total-size, preferred-size);
	sizes[index] := preferred-size
      end
    end
  end;
  when (ratio-denominator > 0)
    let left-over :: <integer> = desired-size - total-size;
    for (index :: <integer> from 0 below n-items)
      unless (sizes[index])
	let ratio = (ratios & index < size(ratios) & ratios[index]) | 1;
	sizes[index] := truncate/(left-over * ratio, ratio-denominator)
      end
    end
  end;
  sizes
