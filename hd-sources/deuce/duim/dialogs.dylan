...
define pane <display-policy-page> ()
  ...
  pane %font-pane (pane)
    make(<spin-box>,
	 items: $deuce-font-sizes,
	 value: pane.%font-size,
	 label-key: first, value-key: second,
	 min-width: 40, width: 40, max-width: 50,
	 value-changed-callback:
	   method (box) pane.%font-size := gadget-value(box) end);
    ...
    vertically (spacing: 2)
      make(<table-layout>,
	   ...
	   contents: vector(vector(make(<label>, label: "&Highlight selection to:"),
				   pane.%marking-pane),
			    vector(make(<label>, label: "&Default font size:"),
				   pane.%font-pane) /*,
...
define sealed method update-pages-from-policy
  ...
  gadget-value(display.%font-pane, do-callback?: #t)
    := default-font-size(policy);
  ...
  with-frame-manager (framem)
    ...
    local method make-policy (dialog)
	    ...
	    make(<editor-policy>,
		 ...
		 default-font-size: display-policy-page.%font-size,
