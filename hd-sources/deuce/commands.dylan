...
define command force-recenter (frame)
  ...
  case
    ...
    state & n < 0 =>
      // Put this line at the (n - 1)'th line from the bottom
      ...
      let n = floor/(vheight, font-height) + n - 2;	// yes, it really is 'n - 2'
      recenter-window(window, line, n);
    otherwise =>
...
define command choose-configuration (frame)
  ...
  when (new-policy)
    ...
    for (window :: <basic-window> in editor-windows(editor))
      ...
      when (show-path-in-title?(old-policy) ~== show-path-in-title?(new-policy))
	display-buffer-name(window, window-buffer(window))
      end
    end
  end
