...
define open abstract class <basic-window> (<window>)
  ...
  sealed slot window-frame :: <editor-state-mixin> = $null-editor-frame,
    init-keyword: frame:;
  // Inter-line spacing
  sealed slot window-line-spacing :: <integer> = 1,
...
define method window-note-policy-changed
    (window :: <basic-window>,
     new-policy :: <editor-policy>, old-policy :: false-or(<editor-policy>)) => ()
  ignore(window, new-policy, old-policy);
  #f
...
define function make-font
    (family, weight, slant, point-size) => (font :: <font>)
  block (return)
    ...
    let font = make(<font>,
		    family: family, weight: weight, slant: slant, size: point-size);
    add!(fonts, font);
    font
  end
...
define variable $default-font :: <font>
    = make-font(#"fix", #"normal", #"roman", #"normal");
define variable $default-bold-font :: <font>
    = make-font(#"fix", #"bold", #"roman", #"normal");
define variable $default-italic-font :: <font>
    = make-font(#"fix", #"normal", #"italic", #"normal");
...
define method set-default-font-size
  ...
  local method merge-size (font :: <font>, size)
	  make-font(font-family(font), font-weight(font), font-slant(font), size)
	end method;
