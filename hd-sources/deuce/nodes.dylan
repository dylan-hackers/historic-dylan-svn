...
define method count-lines
    (interval :: <basic-interval>, #key skip-test = structural-diagram-line?)
 => (nlines :: <integer>)
  let n :: <integer> = 0;
...
define method count-lines
    (string :: <byte-string>, #key skip-test = structural-diagram-line?)
 => (nlines :: <integer>)
  ignore(skip-test);
  count(string, '\n') + 1
end method count-lines;
...
define method do-characters
    (function :: <function>, interval :: <interval>,
     #key start: _start, end: _end, from-end? = #f,
	  skip-test = structural-diagram-line?) => ()
  ignore(_start, _end);
...
define method count-characters
    (interval :: <interval>, #key skip-test = structural-diagram-line?)
 => (nchars :: <integer>)
...
define method count-lines
    (node :: <section-node>, #key skip-test = structural-diagram-line?)
 => (nlines :: <integer>)
