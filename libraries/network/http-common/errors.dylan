...
// Error codes 4xx.  Some 4xx error codes (e.g., 404) aren't really client
...
define http-error method-not-allowed (<http-client-error>)
...
define http-error not-implemented-error (<http-server-error>)
...
// Heh...it might be.
//
define method http-error-code (ex :: <error>)
  $application-error
end;
