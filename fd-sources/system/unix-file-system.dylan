...
define method %expand-pathname (path :: <native-directory-locator>)
...
define method %expand-pathname (path :: <native-file-locator>)
 => (expanded-path :: <locator>)
...
define method %expand-pathname (path :: <locator>) => (expanded-path :: <locator>)
  path
end method %expand-pathname;


///
define function %shorten-pathname (path :: <locator>) => (shortened-path :: <locator>)
  path
end function %shorten-pathname;


///
define function %file-exists? (file :: <locator>) => (exists? :: <boolean>)
...
///
define function %file-type (file :: <locator>, #key if-not-exists = #f)
 => (file-type :: <file-type>)
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
...
///
define function %link-target (link :: <locator>) => (target :: <locator>)
...
///
define function %delete-file (file :: <locator>) => ()
  let file = %expand-pathname(file);
  if (primitive-raw-as-boolean
...
///
define function %file-properties (file :: <locator>)
...
define method %file-property
    (file :: <locator>, key == #"author") => (author :: false-or(<string>))
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
    if (primitive-raw-as-boolean
	  (%call-c-function ("stat") (path :: <raw-byte-string>, st :: <raw-c-pointer>)
...
define method %file-property (file :: <locator>, key == #"size") => (file-size :: <integer>)
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
    if (primitive-raw-as-boolean
	  (%call-c-function ("stat") (path :: <raw-byte-string>, st :: <raw-c-pointer>)
...
define method %file-property
    (file :: <locator>, key == #"creation-date") => (creation-date :: <date>)
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
    if (primitive-raw-as-boolean
	  (%call-c-function ("stat") (path :: <raw-byte-string>, st :: <raw-c-pointer>)
...
define method %file-property
    (file :: <locator>, key == #"access-date") => (access-date :: <date>)
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
    if (primitive-raw-as-boolean
	  (%call-c-function ("stat") (path :: <raw-byte-string>, st :: <raw-c-pointer>)
...
define method %file-property
    (file :: <locator>, key == #"modification-date") => (modification-date :: <date>)
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
    if (primitive-raw-as-boolean
	  (%call-c-function ("stat") (path :: <raw-byte-string>, st :: <raw-c-pointer>)
...
define function accessible? (file :: <locator>, mode :: <integer>)
 => (accessible? :: <boolean>)
  let file = %expand-pathname(file);
  if (primitive-raw-as-boolean
...
define function accessible?-setter
    (new-mode :: <integer>, file :: <locator>, on? :: <boolean>)
 => (new-mode :: <integer>)
  let file = %expand-pathname(file);
  with-stack-stat (st, file)
    if (primitive-raw-as-boolean
...
define method %file-property
    (file :: <locator>, key == #"readable?") => (readable? :: <boolean>)
  accessible?(file, $R_OK)
end method %file-property;

define method %file-property-setter 
    (new-readable? :: <boolean>, file :: <locator>, key == #"readable?")
...
define method %file-property
    (file :: <locator>, key == #"writeable?") => (writeable? :: <boolean>)
  accessible?(file, $W_OK)
end method %file-property;

define method %file-property-setter 
    (new-writeable? :: <boolean>, file :: <locator>, key == #"writeable?")
...
define method %file-property
    (file :: <locator>, key == #"executable?") => (executable? :: <boolean>)
  accessible?(file, $X_OK)
end method %file-property;

define method %file-property-setter 
    (new-executable? :: <boolean>, file :: <locator>, key == #"executable?")
...
define function %do-directory (f :: <function>, directory :: <directory-locator>) => ()
  ...
  block ()
    ...
    while (primitive-machine-word-not-equal?
      ...
      let type :: <file-type> = %file-type(make(<file-locator>,
						directory: directory,
						name: filename));
...
///
define function %create-directory (directory :: <directory-locator>)
...
///
define function %delete-directory (directory :: <directory-locator>) => ()
  let directory = %expand-pathname(directory);
  if (primitive-raw-as-boolean
...
define function %directory-empty? (directory :: <directory-locator>) => (empty? :: <boolean>)
  ~%file-exists?(directory)
    | block (return)
	%do-directory
	  (method (directory :: <directory-locator>, name :: <string>, type :: <file-type>)
...
///
define function %home-directory () => (home-directory :: false-or(<directory-locator>))
...
define function %working-directory () => (working-directory :: false-or(<directory-locator>))
...
///
define function %working-directory-setter (new-working-directory :: <directory-locator>)
 => (new-working-directory :: <directory-locator>)
...
define function %temp-directory () => (temp-directory :: false-or(<directory-locator>))
  *temp-directory*
    | (*temp-directory* :=
	 begin
	   ...
	   if (primitive-machine-word-not-equal?
		 (primitive-unwrap-machine-word(tmpdir),
		  integer-as-raw(0)))
	     let file
	       = as(<file-locator>, primitive-raw-as-string
		                     (primitive-cast-raw-as-pointer
					(primitive-unwrap-machine-word(tmpdir))));
