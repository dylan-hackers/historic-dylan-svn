...
///
define function %expand-pathname (path :: <locator>) => (expanded-path :: <locator>)
...
define function %file-type (file :: <locator>) => (file-type :: <file-type>)
...
define method %file-property
    (file :: <locator>, key == #"creation-date") => (creation-date :: false-or(<date>))
  with-file-attributes (file, fa)
...
define method %file-property
    (file :: <locator>, key == #"access-date") => (access-date :: false-or(<date>))
  with-file-attributes (file, fa)
...
define method %file-property
    (file :: <locator>, key == #"modification-date")
...
define function %do-directory (f :: <function>, directory :: <directory-locator>) => ()
  ...
  let wild-file = make(<file-locator>, directory: directory, name: "*.*");
...
define function %home-directory () => (home-directory :: false-or(<directory-locator>))
  ...
  drive & path
    & as(<directory-locator>, concatenate-as(<string>, drive, path))
...
define function %working-directory () => (working-directory :: false-or(<directory-locator>))
  ...
  if (curdir-size > 0)
    as(<directory-locator>, copy-sequence(curdir-buffer, end: curdir-size))
  else
...
define function %temp-directory () => (temp-directory :: false-or(<directory-locator>))
  with-stack-path (path-buffer)
    ...
    if (path-size ~= 0)
      as(<directory-locator>, copy-sequence(path-buffer, end: path-size))
    else
...
define function %root-directories () => (roots :: <sequence>)
  with-stack-path (path-buffer)
	...
	while (start < path-size)
      ...
      map(curry(as, <directory-locator>), reverse(roots))
    else
