...
define method read-element
 ...
 => (element :: <object>)
  ...
  let limit :: <integer>  = stream-limit(stream) | seq.size;
  if (pos < limit)
    let elt = seq[pos];
...
define method unread-element
 ...
 => (element :: <object>)
  ...
  if (pos > 0)
    stream.current-position := pos - 1;
    stream-sequence(stream)[pos - 1] := elt
  end;
  elt
...
define method read 
 ...
 => (elements :: <sequence>)
  ...
  let seq   :: <sequence> = stream-sequence(stream);
  let pos   :: <integer>  = stream.current-position;
  let src-n :: <integer>  = (stream-limit(stream) | seq.size) - pos;
  if (n > src-n)
...
define method stream-at-end?
  ...
  if (write-only?(stream))
    ...
    stream.current-position >= (limit | size(stream-sequence(stream)))
  end
