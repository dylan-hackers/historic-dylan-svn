  ...
  case
    ...
    otherwise =>
      ...
      block (return)
        while (line-number < -1)
          let next-line 
            = line-next-in-buffer(last-line, buffer, skip-test: #f);
          if (next-line)
            last-line := next-line
          else
            return(last-line)
 ...
 => (line-number :: <integer>)
  ...
  block (return)
    until (line == first-line)
      let next-line = line-next-in-buffer(first-line, buffer, skip-test: #f);
      if (next-line)
 ...
 => ()
  with-editor-state-bound (buffer = window)
    ...
    block (return)
      ...
      if ($debug-redisplay?)
        debug-message("Redisplay degree: %=", degree)
      end;
...
define constant $duim-kludge-fudge = 1;
...
define sealed method update-caret-position
 ...
 => (visible? :: <boolean>)
  ...
  when (line-for-display-only?(line))
    ...
    line := bp-line(bp);
    if (line-for-display-only?(line) & $debug-redisplay?)
  ...
  case
	...
	= case
      ...
      update-caret-position(window, buffer, move-point?: #f);
    otherwise =>
      ...
      if ($debug-redisplay?)
	let frame = window-frame(window);
	if (scrolling-moves-point?(editor-policy(frame-editor(frame))))
 ...
 => (bp :: false-or(<basic-bp>), display-line :: false-or(<display-line>))
  ...
  when (buffer)
    ...
    block (return)
      ...
      let n-lines :: <integer> = window-n-display-lines(window);
      let last-y = 0;
      if (n-lines > 0 & y < lines[0].display-line-y)
	dline := lines[0];
	return();
      end;
      without-bounds-checks
...
define sealed method move-point!
  ...
  when (line-for-display-only?(line))
    ...
    //break("Moving point to display-only line");
  end;
